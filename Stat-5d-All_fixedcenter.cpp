...
//modified 12/14/22
//read in 4D packing, computing the corresponding quantities, starting with Sk first ....

//modified 12/19/22
//generalized to 5D packing, only Sk works
...
#define Bin 0.0075  //these should chosen accordingly 
...
int Nk = 10; //this is the number of mesh point along each direction in K space, the unit of mesh is 2pi/L
...
double MinDis(int m, int n)
{
  ...
  double dg = fabs(Center[m][4] - Center[n][4]);
  
  if(dx >= L[0]/2.0) dx = L[0] - dx;
  if(dy >= L[1]/2.0) dy = L[1] - dy;
  if(dz >= L[2]/2.0) dz = L[2] - dz;
  if(dh >= L[3]/2.0) dh = L[3] - dh;
  if(dg >= L[4]/2.0) dg = L[4] - dg;
  
  	
  //cout<<sqrt(dx*dx + dy*dy + dz*dz + dh*dh + dg*dg)<<endl;
  	
  return sqrt(dx*dx + dy*dy + dz*dz + dh*dh + dg*dg); 
...
void Get_PairCorr()
{
  ...
  for(int r = 0; r < Nbin; r++)
    {
      g[r] = g[r] / (N *rho_n*pow(pi, 5/2)*((r+1.0)*(r+1.0)*(r+1.0)*(r+1.0)*(r+1.0) - r*r*r*r*r)*Bin*Bin*Bin*Bin*Bin/(2.5*1.5*0.5));
    }
...
void get_contact_number()
{
	...
	int gap_pow = -11; //this is the smallest gap pow 
	
	int n_contact[10]; //the dimension gives the number of gap
	int num_gap = 10; //must be the same as the size of n_contact
	
	for(int i=0; i<num_gap; i++)
		n_contact[i] = 0;
		
	int cn_counter[N][10]; //the number of contact number for each particle, for rattler statistics
	for(int i=0; i<N; i++)
	...
	fout.open("n_contact_N.xls");
	fout<<"the average contact NOT exclude rattlers!"<<endl;
	for(int m=0; m<num_gap; m++)
		fout<<pow(10, gap_pow+m)<<"\t"<<((double)n_contact[m])/(double)N<<endl;
	fout.close();
	
	fout.open("n_contact_Nb.xls");
	fout<<"the average contact already exclude rattlers!"<<endl;
	for(int m=0; m<num_gap; m++)
		fout<<pow(10, gap_pow+m)<<"\t"<<((double)n_contact[m])/(double)N_gap[m]<<endl;
	fout.close();
	
	fout.open("N_backbond.xls");
	fout<<"the number of particles in the backbone!"<<endl;
	for(int m=0; m<num_gap; m++)
		fout<<pow(10, gap_pow+m)<<"\t"<<N_gap[m]<<endl;
	fout.close();
	
	fout.open("N_contact_ALL.xls");
	fout<<"the total number of contacts for all particles!"<<endl;
	for(int m=0; m<num_gap; m++)
		fout<<pow(10, gap_pow+m)<<"\t"<<n_contact[m]<<endl;
	fout.close();
