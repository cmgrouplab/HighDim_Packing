...
int Nk = 18; //this is the number of mesh point along each direction in K space, the unit of mesh is 2pi/L
double Kbin = 6; //this should be on the magnitude of 2pi/L 
                   //tried values 2, 3, 4, 6; Kbin can significantly affect small k behavior ...
#define Kbin_num 350
...
void Get_PairCorr()
{
  ...
  for(int r = 0; r < Nbin; r++)
    {
      g[r] = g[r] / (N * rho_n * 0.5*pi*pi * ((r+1.0)*(r+1.0)*(r+1.0)*(r+1.0) - r*r*r*r) * Bin * Bin * Bin * Bin);
    }
...
void get_contact_number()
{
	...
	int gap_pow = -12; //this is the smallest gap pow 
